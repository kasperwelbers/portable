...
onmessage = (event) => {
  const { eventType, host } = event.data;
  switch (eventType) {
    case "initialise":
      initialise(host).then(() => {
...
function initialise(host) {
  ...
  return startPyodide()
    ...
    .then(() => {
      return installPortPackage(host);
    });
}
...
function installPortPackage(host) {
  console.log("[ProcessingWorker] load port package");

  return self.pyodide.runPythonAsync(`
    import micropip
    await micropip.install("${host}/port-0.0.0-py3-none-any.whl", deps=False)
    import port
  `);
}
